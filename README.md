...
> `Spring Cloud Alibaba`、`Nacos`、`Sentinel`、`OpenFeign`、`Gateway`、`RocketMQ`、日志链路追踪等。  
...
六、日志链路追踪
加依赖（如果是新版 Spring Cloud，推荐用 Micrometer Tracing）：
...
然后在 resources 下放一个 logback-spring.xml，配置好日志格式，增加 traceId 和 spanId 输出：

xml
<property name="LOG_PATTERN" value="%d{yyyy-MM-dd HH:mm:ss.SSS} [%thread] %-5level [%X{traceId}] [%X{spanId}] - %msg%n"/>
这样控制台打印的日志就会带上 traceId，一个请求的所有日志 traceId 相同，spanId 表示不同的处理片段（比如查 Redis、写数据库）。
异步线程或 RocketMQ 消费端一般框架会自动传递，不用太操心。
